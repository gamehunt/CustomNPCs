...
namespace NPCS
{
    public class EventHandlers
    {
        ...
        {
            Log.Info("Generating navigation graph...");
            foreach (Room r in Map.Rooms)
            {
                NavigationNode node = NavigationNode.Create(r.Position, $"NavNode_Autogenerated_Room_{r.Name}".Replace(' ', '_'));
                foreach (Door d in r.Doors)
                {
                    ...
                    NavigationNode new_node = NavigationNode.Create(d.gameObject.transform.position, $"NavNode_Autogenerated_Door_{(d.DoorName.IsEmpty() ? d.gameObject.transform.position.ToString() : d.DoorName)}".Replace(' ', '_'));
                    if (new_node == null)
                    {
                        new_node = NavigationNode.AllNodes[$"NavNode_Autogenerated_Door_{(d.DoorName.IsEmpty() ? d.gameObject.transform.position.ToString() : d.DoorName)}".Replace(' ', '_')];
                    }
                    node.LinkedNodes.Add(new_node);
                    new_node.LinkedNodes.Add(node);
                    Log.Debug($"Linked door {new_node.Name} node to room {r.Name}", Plugin.Instance.Config.VerboseOutput);
                }
            }
        }
