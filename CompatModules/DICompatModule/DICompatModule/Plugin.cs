...
{
    public class Plugin : Exiled.API.Features.Plugin<DICompatModule.Config>
    {
        public override string Author { get; } = "gamehunt";
        public override string Name { get; } = "DICompatModule";
        public override string Prefix { get; } = "DICompatModule";
        ...
        public override void OnEnabled()
        {
            try
            {
                ...
                if (NPCPlugin == null)
                {
                    Log.Error("Failed to load DI compat module addon: DI not found!");
                    return;
                }

                Harmony = new HarmonyLib.Harmony("gamehunt.compat.di");
                Harmony.PatchAll();

                Log.Info($"DI compat module loaded. @gamehunt");
            }
            catch (Exception e)
            {
