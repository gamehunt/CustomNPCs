...
namespace UAfkCompatModule
{
    public class Plugin : Exiled.API.Features.Plugin<AdminToolsCompatModule.Config>
    {
        public override string Author { get; } = "gamehunt";
        public override string Name { get; } = "AdminToolsCompatModule";
        public override string Prefix { get; } = "AdminToolsCompatModule";
        ...
        public override void OnEnabled()
        {
            try
            {
                ...
                if (NPCPlugin == null)
                {
                    Log.Error("Failed to load AdminTools compat module addon: CustomNPCs not found!");
                    return;
                }

                if (Exiled.Loader.Loader.Plugins.Where(p => p.Prefix == "AT").FirstOrDefault() == null)
                {
                    Log.Error("Failed to load AdminTools compat module addon: AdminTools not found!");
                    return;
                }

                Harmony = new HarmonyLib.Harmony("gamehunt.compat.at");
                Harmony.PatchAll();

                Log.Info($"AdminTools compat module loaded. @gamehunt");
            }
            catch (Exception e)
            {
